...
def main():
    ...
    df = pd.read_csv('first_year_female_prediction.csv')
    ########## dataset ##########

    # drop useless columns #
    train_df1 = df.iloc[:, :33]
    train_df2 = df.iloc[:, 50]
    train_df3 = df.iloc[:, 53:57]
    train_df = pd.concat([train_df1, train_df2, train_df3], axis=1)
    ...
    for i in range(4):
        titles[22+i], titles[22+i+1] = titles[22+i+1], titles[22+i]
