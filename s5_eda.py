...
Author: Alex
-----------------------------------------------------
original training set + multi-cat (for scenario 5)
"""
...
def main():
    ########## dataset ##########
    df = pd.read_csv('Data/NIJ_s_Recidivism_Challenge_Training_Dataset_With_PUMA_&_Multi_Category_Features.csv')
    ########## dataset ##########

    # drop useless columns #
    train_df1 = df.iloc[:, :5]
    train_df2 = df.iloc[:, 22:66]
    train_df3 = df.iloc[:, 70:]
    train_df4 = df.iloc[:, 67:69]
    train_df = pd.concat([train_df1, train_df2, train_df3, train_df4], axis=1)
    ...
    train_df.loc[(df.Recidivism_Arrest_Year1 == 1), 'Three_Cat_Labels'] = 2
    train_df.loc[(df.Recidivism_Arrest_Year1 == 0) & (df.Recidivism_Arrest_Year2 == 0), 'Three_Cat_Labels'] = 0
    train_df.loc[(df.Recidivism_Arrest_Year1 == 0) & (df.Recidivism_Arrest_Year2 == 1), 'Three_Cat_Labels'] = 1
    ...
    # train = train[train['Gender'] == -1]
    # train.to_pickle('Local_F_mean_imputation_multi_cat.pkl')
    train.to_pickle('Processed_Data/s5.pkl')
    ########## three year recidivism data ###########
    print('#'*50 + ' end ' + '#'*50)
