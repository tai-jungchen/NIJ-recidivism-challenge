...
def main():
    ...
    for i in tqdm(range(len(ran_state))):
        ########## sampling ##########
        X_train_y1y2, X_val_y1y2, y_train, y_val = train_test_split(X, y, test_size=0.4, stratify=X[:, 0],
                                                          random_state=ran_state[i])
        X_train = X_train_y1y2[:, :-2]     # drop year_1 column and year_2 column
        y_train = y_train.astype('int')
        Xy_train_y1y2 = np.concatenate((X_train_y1y2, y_train[:, np.newaxis]), axis=1)

        # get only female and year 1 recidivism != 1 #
        y_val = np.expand_dims(y_val, axis=1)
        Xy_val_y1y2 = np.concatenate((X_val_y1y2, y_val), axis=1)
        Xy_val_y1y2 = Xy_val_y1y2[Xy_val_y1y2[:, 0] == -1]     # discard male
        Xy_val_y1y2 = Xy_val_y1y2[Xy_val_y1y2[:, -3] != 1]     # discard year 1 positive subject
        X_val = Xy_val_y1y2[:, :-3]
        y_val = Xy_val_y1y2[:, -1]
        ...
        Xy_val_y1y2_p = np.concatenate((Xy_val_y1y2, y_pred[:, np.newaxis], y_prob), axis=1)
        y_prob_new, y_pred_new, y_true_new, X_test = ensemble_35(Xy_val_y1y2_p, Xy_train_y1y2)
...
def ensemble_35(Xy_val_y1y2_p, Xy_train_y1y2):
    ...
    y_pred_3 = model_3.predict(Xy_val_3[:, :-7])
    y_prob_3 = model_3.predict_proba(Xy_val_3[:, :-7])
    ########## s3 model ##########
    diging(y_prob_3, y_val_3, y_pred_3, Xy_val_3)
    ########## s5 model ##########
    ...
    y_pred_5 = model_5.predict(Xy_val_5[:, :-7])
    y_prob_5 = model_5.predict_proba(Xy_val_5[:, :-7])
    ...
    return y_prob, y_pred, y_true, Xy_val_y1y2_p
