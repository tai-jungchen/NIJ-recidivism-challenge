...
def main():
    ...
    # df_global = df_global[df_global['Gender'] == 1]     # select male only
    df_global = df_global[df_global['Gender'] == -1]  # select female only
    ...
    for i in tqdm(range(len(ran_state))):
        ...
        # get only male and year 1 recidivism != 1 #
...
def diging(y_prob, y_true):
    ...
    :return:
    """
    y_prob_fp = y_prob[np.where(y_prob[:, 2] > 0.5)]
    y_true_fp = y_true[np.where(y_prob[:, 2] > 0.5)]
    ...
    plt.legend(['probability of not recidivating in year 2', 'probability of recidivating in year 2',
                'True answer of whether the subject recidivate'])
    plt.show()
