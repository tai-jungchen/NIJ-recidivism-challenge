...
Author: Alex
-----------------------------------------------------
Testing data (original + multi-cat version)
"""
...
def main():
    df = pd.read_csv('Data/NIJ_s_Recidivism_Challenge_Test_Dataset3_With_PUMA_&_Multi_Category_Features.csv')
    ...
    train_df_1 = df.iloc[:, :5]
    train_df_2 = df.iloc[:, 22:]
    train_df = pd.concat([train_df_1, train_df_2], axis=1)
    ...
    train = train[train['Gender'] == -1]
    train.to_pickle('third_year_testing.pkl')
    # train.to_pickle('Global_mean_imputation_multi_cat.pkl')
    ########## mean imputation ###########
