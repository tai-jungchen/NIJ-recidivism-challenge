...
def main():
    ...
    puma = pd.read_csv('Data/PUMA_Breakdown.csv')
    puma = puma.dropna()
    puma = puma.drop(['NIJ_Residence_PUMA'], axis = 1)
    ...
    # add new features #
    new_features = list(puma.columns)
    values = np.random.rand(df.shape[0], 1)
    for feature in new_features:
        train_df[feature] = values
    titles = list(train_df.columns)
    for i in range(len(new_features)):
        titles[33+i], titles[33+i+1] = titles[33+i+1], titles[33+i]
    train_df = train_df[titles]     # swap label to the end
    puma_codes = list(train_df['Residence_PUMA'])
    for i in tqdm(range(train_df.shape[0])):
        train_df.iloc[i, 33:train_df.shape[1]-1] = list(puma.iloc[puma_codes[i]-1])
    ...
    # drop original PUMA feature
    train_df = train_df.drop(['Residence_PUMA'], axis=1)
    ...
    # train = train[train['Gender'] == -1]
    # train.to_pickle('Local_F_mean_imputation_new_PUMA.pkl')
    ########## mean imputation ###########
    ...
    ########## for PUMA cat ###########
    train_df.to_pickle('for_PUMA_cat.pkl')
    ########## for PUMA cat ###########
