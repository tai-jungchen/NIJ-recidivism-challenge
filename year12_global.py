...
Using the year 1 + year 2 as labels
"""
...
def main():
    df = pd.read_pickle("Processed_Data/Global_MI_multi_cat_year12_2.pkl")
    ...
    for ran_state in ran_states:
        ...
        X = data[:, 1:39]
        y_12 = data[:, - 1]
        y_12 = y_12.astype('int')
        y_12_pred, _ = predict(X, y_12, ran_state)

        # predict year1
        y_1 = data[:, - 3]
        y_1 = y_1.astype('int')
        y_1_pred, y_1_val = predict(X, y_1, ran_state)

        # get y_val for year 2
        y_2 = data[:, - 2]
        y_2 = y_2.astype('int')
        y_2_dpred, y_val = predict(X, y_2, ran_state)

        # cancel out year1
        y_2_pred = y_12_pred - y_1_val
        acc = np.sum(y_2_pred == y_val) / len(y_2_pred)
        acc_direct = np.sum(y_2_dpred == y_val) / len(y_2_dpred)
        accs.append(acc)
        accs_d.append(acc_direct)
        print(f'*****Accuracy of year12 model: {acc}')
        print(f'*****Accuracy of year2 model: {acc_direct}')

    print('#' * 50 + ' global summary' + '#' * 50)
    print(f'mean accuracy of year 12 method: {stat.mean(accs)}; se: {stat.stdev(accs) / math.sqrt(10)}')
    print(f'mean accuracy of direct method: {stat.mean(accs_d)}; se: {stat.stdev(accs_d) / math.sqrt(10)}')
    print('#' * 50 + ' end ' + '#' * 50)


def predict(X, y, ran_state):
    """
    This function carry out the prediction task
    :param X: (nparray) features
    :param y: (nparray) labels
    return: (list) predictions
    """
    ########## oversampling on gender ##########
    X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=0.4, stratify=X[:, 0], random_state=ran_state)
