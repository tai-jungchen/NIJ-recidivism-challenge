...
Author: Alex
-----------------------------------------------------
original training set + multi-cat + three cat (for scenario 0-8)
"""
...
def main():
    ...
    train_df = df.drop(['Prior_Arrest_Episodes_DVCharges', 'Prior_Arrest_Episodes_GunCharges',
    ...
    train_df_1 = train_df.iloc[:, :34]
    train_df_2 = train_df.iloc[:, 38:]
    train_df_3 = train_df.iloc[:, 34:38]
    train_df = pd.concat([train_df_1, train_df_2, train_df_3], axis=1)
    ...
    # 49. Employment_Exempt
    train_df.loc[(df.Employment_Exempt == False), 'Employment_Exempt'] = 0
    train_df.loc[(df.Employment_Exempt == True), 'Employment_Exempt'] = 1
    # 50. Recidivism_Within_3years
    train_df.loc[(df.Recidivism_Within_3years == False), 'Recidivism_Within_3years'] = 0
    train_df.loc[(df.Recidivism_Within_3years == True), 'Recidivism_Within_3years'] = 1
    ...
    train_df.loc[(df.Recidivism_Arrest_Year2 == False), 'Recidivism_Arrest_Year2'] = 0
    train_df.loc[(df.Recidivism_Arrest_Year2 == True), 'Recidivism_Arrest_Year2'] = 1
    # 53. Recidivism_Arrest_Year3
    train_df.loc[(df.Recidivism_Arrest_Year3 == False), 'Recidivism_Arrest_Year3'] = 0
    train_df.loc[(df.Recidivism_Arrest_Year3 == True), 'Recidivism_Arrest_Year3'] = 1
    # Set1_Prior_Arrest
    ...
    # new feature = 3-category label
    three_cat = np.zeros(len(train_df))
    train_df['Three_Cat_Labels'] = three_cat
    train_df.loc[(df.Recidivism_Arrest_Year1 == 1) | (df.Recidivism_Arrest_Year2 == 1), 'Three_Cat_Labels'] = 2
    train_df.loc[(df.Recidivism_Arrest_Year1 == 0) & (df.Recidivism_Arrest_Year2 == 0) & (df.Recidivism_Arrest_Year3 == 0),
                 'Three_Cat_Labels'] = 0
    train_df.loc[(df.Recidivism_Arrest_Year1 == 0) & (df.Recidivism_Arrest_Year2 == 0) & (df.Recidivism_Arrest_Year3 == 1),
                 'Three_Cat_Labels'] = 1
    # new feature = year1 + year2 label
    Recidivism_Arrest_Year12 = train_df['Recidivism_Arrest_Year1'] + train_df['Recidivism_Arrest_Year2']
    train_df['Recidivism_Arrest_Year12'] = Recidivism_Arrest_Year12
    ########## new feature = 3-category label for first two years ##########
    three_cat_2 = np.zeros(len(train_df))
    train_df['Three_Cat_Labels_2'] = three_cat_2
    train_df.loc[(df.Recidivism_Arrest_Year1 == 1), 'Three_Cat_Labels_2'] = 2
    train_df.loc[(df.Recidivism_Arrest_Year1 == 0) & (df.Recidivism_Arrest_Year2 == 0), 'Three_Cat_Labels_2'] = 0
    train_df.loc[(df.Recidivism_Arrest_Year1 == 0) & (df.Recidivism_Arrest_Year2 == 1), 'Three_Cat_Labels_2'] = 1
    ########## new feature = 3-category label for first two years ##########
    ...
    # train.to_pickle('Processed_Data/stage3_df.pkl')
    train.to_pickle('Processed_Data/stage3_s8_df.pkl')
    ########## three year recidivism data ###########
    print('#'*50 + ' end ' + '#'*50)
