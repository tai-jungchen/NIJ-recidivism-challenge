...
def ensemble_35(Xy_val_y3_p, Xy_train_y3):
    ...
    """
    Xy_val_3 = Xy_val_y3_p[Xy_val_y3_p[:, -4] == 2]        # select the false positive cases
    Xy_val_5 = Xy_val_y3_p[Xy_val_y3_p[:, -4] != 2]        # select the FF and FT
    Xy_val_35 = np.concatenate((Xy_val_3, Xy_val_5), axis=0)
    y_val_3 = Xy_val_3[:, -6]
    y_val_3 = y_val_3.astype('int')
    y_val_5 = Xy_val_5[:, -5]
    y_val_5 = y_val_5.astype('int')
    ########## s3 model ##########
    Xy_train_y3_3= Xy_train_y3[Xy_train_y3[:, -1] != 2]       # drop Recidivism_Arrest_Year1 2 positive cases
    X_train_3 = Xy_train_y3_3[:, :-2]       # drop Recidivism_Arrest_Year1 column
    y_train_3 = Xy_train_y3_3[:, -2]
    model_3 = global_model(X_train_3, y_train_3)
    y_pred_3 = model_3.predict(Xy_val_3[:, :-6])
    y_prob_3 = model_3.predict_proba(Xy_val_3[:, :-6])
    ########## s3 model ##########
    ...
    model_5 = global_model(X_train_5, y_train_5)
    y_pred_5 = model_5.predict(Xy_val_5[:, :-6])
    y_prob_5 = model_5.predict_proba(Xy_val_5[:, :-6])
    y_prob_5_bin = y_prob_adj(y_prob_5)
    ########## s5 model ##########

    diging(y_prob_3, y_val_3, y_pred_3, Xy_val_3, y_prob_5, y_val_5, y_pred_5, Xy_val_5)

    ########## combine results ##########
    y_prob = np.concatenate((y_prob_3, y_prob_5_bin), axis=0)
    y_pred = np.concatenate((y_pred_3, y_pred_5), axis=0)
    y_true = np.concatenate((y_val_3, y_val_5), axis=0)
    ########## combine results ##########
...
def diging(y_prob_3, y_val_3, y_pred_3, Xy_val_3, y_prob_5, y_val_5, y_pred_5, Xy_val_5):
    """
    ...
    ax1.plot(y_prob_3[:, 1])
    ax1.plot(y_val_3, 'o')
    ax1.set_title('false positive group on scenario 3')

    ax2.plot(y_prob_5[:, 0])
    ax2.plot(y_prob_5[:, 1])
    ax2.plot(y_val_5, 'o')
    ax2.set_title('false positive group on scenario 5')
    ...
    metrics_c(y_prob_3, y_val_3, y_pred_3, Xy_val_3)
    # print(f'\n********** metrics for non false positive on scenario 5 **********')
    # metrics_c(y_prob_5, y_val_5, y_pred_5, Xy_val_5)
